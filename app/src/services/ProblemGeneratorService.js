...
## 문제 작성 규칙 (매우 중요)
- question은 반드시 **한글**로 작성 (수학 변수 x, y, a, b와 수식 기호 +, -, =, >, < 만 허용)
- LaTeX 문법 절대 금지 (\\frac, \\sqrt, \\times, \\div, \\pi, \\alpha 등 사용 금지)
- 제곱은 x^2 또는 "x의 제곱"으로, 분수는 1/2 또는 "2분의 1"로 작성
- 루트는 "루트3" 또는 "√3"으로 작성
- 영어 문장이나 영어 설명 금지 (예: "Solve for x" → "x의 값을 구하시오")
- 마크다운 서식 사용 금지 (볼드, 이탤릭, 코드블록 등)
...
## 오답 생성 규칙 (매우 중요)
- 오답은 반드시 정답과 **같은 형식**이어야 합니다
  - 정답이 텍스트면 오답도 텍스트, 정답이 숫자면 오답도 숫자
  - 정답에 단위가 있으면 오답에도 같은 단위 사용 (예: 정답 "10 m/s²" → 오답 "5 m/s²")
- 오답은 정답과 **비슷한 글자 수/길이**여야 합니다
- 오답은 정답과 **같은 과학 분야의 관련 용어**여야 합니다
- 4개 선택지만 보고 정답을 찍을 수 없도록, 오답도 그럴듯해야 합니다

### 나쁜 예시 (절대 이렇게 만들지 마세요)
- 정답 "미토콘드리아" → 오답 "42", "87", "15" (X) ← 텍스트 답에 숫자 오답 금지
- 정답 "DNA" → 오답 "사과", "컴퓨터", "축구" (X) ← 무관한 단어 금지
- 정답 "10 m/s²" → 오답 "속력", "변위", "질량" (X) ← 숫자+단위 답에 텍스트 오답 금지

### 좋은 예시 (주제별)
- 힘과 운동: 정답 "가속도" → 오답 "속력", "변위", "관성"
- 세포: 정답 "미토콘드리아" → 오답 "리보솜", "소포체", "골지체"
- 화학: 정답 "질량 보존 법칙" → 오답 "에너지 보존 법칙", "일정 성분비 법칙", "배수 비례 법칙"
- 유전: 정답 "DNA" → 오답 "RNA", "mRNA", "단백질"
- 계산: 정답 "20 N" → 오답 "10 N", "40 N", "5 N"
...
## 문제 작성 규칙 (매우 중요)
- question은 반드시 **한글**로 작성 (과학 약어 DNA, RNA, pH, mol 등은 허용)
- LaTeX 문법 절대 금지 (\\frac, \\sqrt, \\times 등 사용 금지)
- 화학식은 간단히 작성 (예: H2O, CO2, NaCl)
- 영어 문장이나 영어 설명 금지 (예: "What is..." → "~은 무엇인가?")
- 마크다운 서식 사용 금지 (볼드, 이탤릭, 코드블록 등)
...
${answerRule}
- 단계별 풀이 포함
- question은 한글로 작성 (변수 x, y와 수식 기호만 허용, LaTeX 금지, 영어 문장 금지)
- question에는 순수 문제 텍스트만 (영어 번역, 번호, 메타정보, 마크다운 서식 제외)
