...
### 6. `_aiGenerating` 락 영구 고착 버그 수정 (2차 수정)
- **근본 원인**: finally 블록에서 `_aiGenerating = false`가 **마지막 줄**에 위치
  - `changeScreen(SCREENS.MAIN)` → `SoundService.init()` 등에서 에러 발생 시
  - `_aiGenerating = false`에 도달하지 못해 **영구 락**
  - 이후 "AI 문제 생성" 버튼 클릭 → `if (this._aiGenerating) return;` → 무시됨
- **수정**:
  - `_aiGenerating = false`를 finally 블록 **첫 줄**로 이동
  - `_safeCleanup()` 메서드 추가: 정리 코드를 try/catch로 감싸 에러가 나도 최소한의 상태 복구
  - `_finishAndShowResult()`의 `changeScreen` 호출도 try/catch 래핑
  - 잔류 모달(`custom-modal`) DOM 제거 추가

```
// 수정 전 (에러 시 락 영구 고착)
finally {
    game.changeScreen(MAIN);   ← 여기서 에러 시
    this._aiGenerating = false; ← 도달 못함!
}

// 수정 후 (항상 락 해제)
finally {
    this._aiGenerating = false; ← 먼저 해제
    this._safeCleanup(game);    ← try/catch로 안전하게 정리
}
```

## 검증
- `npm run build` 성공
- `npx vitest run` 전체 통과 (8 파일, 215 테스트)
  - 새 테스트 2개: changeScreen 에러 시에도 락 해제 검증
